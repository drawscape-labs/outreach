...
export function DropdownItem({
  ...
  const classes = cn(
    ...
    // Text styles
    'text-left text-base/6 text-zinc-950 dark:text-zinc-100 sm:text-sm/6 forced-colors:text-[CanvasText]',
    // Focus
...
export function DropdownHeading({
  ...
  return (
    <Headless.MenuHeading
      ...
      className={cn(
        className,
        'col-span-full grid grid-cols-[1fr_auto] gap-x-12 px-3.5 pt-2 pb-1 text-sm/5 font-medium text-zinc-500 dark:text-zinc-400 sm:px-3 sm:text-xs/5'
      )}
    />
  )
}
...
export function DropdownShortcut({
  ...
  const keyParts = Array.isArray(keys) ? keys : keys.split('')
  const seenKeys = new Map<string, number>()
  const renderedKeys = keyParts.map((char) => {
    const occurrence = (seenKeys.get(char) ?? 0) + 1
    seenKeys.set(char, occurrence)
  ...
  return (
    ...
    >
      {renderedKeys.map(({char, id}, index) => (
        <kbd
          key={id}
          className={cn([
            'min-w-[2ch] text-center font-sans text-zinc-400 capitalize group-data-focus:text-white dark:text-zinc-500 forced-colors:group-data-focus:text-[HighlightText]',
