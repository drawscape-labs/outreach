---
name: hunter-io
description: Use the Hunter.io API for outreach research. Use when the user asks to search a domain, find or verify a work email, enrich a person or company, test the Hunter key, or prepare Hunter-backed evidence before outreach.
compatibility: Requires internet access and Node.js 20+. Live requests require HUNTER_API_KEY in the repository-root .env file.
---

# Hunter.io

Use Hunter.io as a secondary prospecting and verification source for this outreach workspace.

In commands below, set `SKILL_DIR` to the directory containing this `SKILL.md`. Keep the repository root as the current working directory so the root `.env` is discovered.
...
- Treat Hunter as secondary evidence. For database updates, preserve the `find-person-email` skill rule: save only exact work emails after explicit approval, and prefer direct public source evidence when available.
...
- Put generated Hunter files under `.agent-runs/hunter-io/<domain-person-or-run>/`.
...
node "$SKILL_DIR/scripts/hunter-api.js" domain-search --domain example.com --department sales --type personal --limit 10 --summary
node "$SKILL_DIR/scripts/hunter-api.js" email-finder --domain example.com --first-name Jane --last-name Doe --summary
node "$SKILL_DIR/scripts/hunter-api.js" email-verifier --email jane@example.com --summary
node "$SKILL_DIR/scripts/hunter-api.js" enrich-company --domain example.com --summary
```
...
2. Choose the endpoint:
   - Use `domain-search` to discover known emails at a company. For person prospecting, start with `--type personal --department sales --required-field full_name,position`.
