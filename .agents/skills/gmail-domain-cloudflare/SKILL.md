...
description: Set up a Gmail or Google Workspace domain through Cloudflare DNS. Use when adding Google verification, MX, SPF, DKIM, or DMARC records, optionally redirecting a secondary domain, or confirming DNS propagation.
compatibility: Requires internet access, Node.js 20+, a Cloudflare API credential, and values from the user's Google Workspace setup.
---
...
- Before writing DNS records, read `../cloudflare-dns/SKILL.md` if its Cloudflare token/API helper is not already in context.
...
- For host values shown as "default value", `@`, or blank, use the zone apex, for example `<domain>`.
- Query existing records before writes, avoid exact duplicates, then verify through both Cloudflare API and public DNS.

## Working Directory

- Put generated setup files under `.agent-runs/gmail-domain-cloudflare/<domain>/`.
...
- `name` is the domain apex, for example `<domain>`.
...
6. Configure the Cloudflare redirect to the primary domain:
   - Redirect the apex and `www` hostnames to `https://<primary-domain>`.
...
For secondary domains such as `<secondary-domain>`, set up a Cloudflare Redirect Rule so web traffic forwards to `https://<primary-domain>`.
...
```json
{
  "description": "Redirect domain to <primary-domain>",
  "expression": "(http.host eq \"<domain>\" or http.host eq \"www.<domain>\")",
  "action": "redirect",
  "action_parameters": {
    "from_value": {
      "target_url": {
        "expression": "concat(\"https://<primary-domain>\", http.request.uri.path)"
      },
      "status_code": 301,
...
curl -I http://<domain>/test-path?agent-test=1
curl -I https://<domain>/test-path?agent-test=1
curl -I http://www.<domain>/test-path?agent-test=1
curl -I https://www.<domain>/test-path?agent-test=1
```

Expected HTTP result:

- Status: `301`
- `Location`: `https://<primary-domain>/test-path?agent-test=1`

Example result for `<secondary-domain>`:

- Created `http_request_dynamic_redirect` entrypoint with one rule.
- Rule expression: `(http.host eq "<secondary-domain>" or http.host eq "www.<secondary-domain>")`.
- All four curl variants returned `301` to `https://<primary-domain>/test-path?agent-test=1`.
...
- Cloudflare API calls use the repo-local token loading pattern from `../cloudflare-dns/SKILL.md`.
...
- Redirect target is `https://<primary-domain>`.
...
- Redirect status, including proxied hostname records and target `https://<primary-domain>`.
