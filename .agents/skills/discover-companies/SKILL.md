...
Find companies that match a user-provided audience prompt and the repository's configured outreach strategy, import new company rows through the web app API, then use the `enrich-company` skill for newly inserted records.
...
- Do not set `date_enriched` during discovery import. The `enrich-company` skill owns enrichment completion and date stamping.
...
Save each run under `.agent-runs/discover-companies/<run-label>/`.
...
3. Qualify candidates:
   ...
   - Prefer accounts with visible people matching the personas associated with that industry so the `prospect-company` skill has useful contacts to pursue.
...
```bash
node "$SKILL_DIR/scripts/import-discovered-companies-api.js" .agent-runs/discover-companies/<run-label>/candidates.json --api-base http://localhost:4200
```
...
```bash
node "$SKILL_DIR/scripts/import-discovered-companies-api.js" .agent-runs/discover-companies/<run-label>/candidates.json --api-base http://localhost:4200 --apply
```
...
7. Enrich new companies:
   - For every result in `affected_companies`, immediately use the `enrich-company` skill.
   - Pass the inserted company id, domain, LinkedIn company URL, and discovery notes as input context.
   - Let the `enrich-company` skill perform API-backed updates, fill richer fields, and set `date_enriched` after enrichment succeeds.
