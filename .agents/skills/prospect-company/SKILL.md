...
compatibility: Requires internet access, Node.js 20+, and the local outreach web app for database imports.
---
...
- If the API is unreachable from the current agent run, stop before import and report the API connectivity failure. Do not fall back to Prisma, `sqlite3`, or direct SQLite writes.
...
- Apply the `find-person-email` skill rules whenever researching emails: work emails only, company website first, exact public evidence only, no personal emails, no pattern-only guesses, no SMTP probing without approval.
...
- Save all generated prospecting files under `.agent-runs/prospect-company/`.
- Start each run by creating a per-company run directory: `node "$SKILL_DIR/scripts/create-run-dir.js" --label "<company or domain>"`.
...
- If the user provides a final artifact path, also keep a working copy and command outputs in the `.agent-runs/prospect-company/` run directory.

## Workflow

1. Create the run directory:
   - Run `node "$SKILL_DIR/scripts/create-run-dir.js" --label "<company or domain>"`.
...
2. Normalize input:
   ...
   - Run `node "$SKILL_DIR/scripts/canonicalize-input.js" <input.json>` when useful.
...
5. Find candidate people:
   ...
   - Never derive or save an email from an observed company pattern. If only a pattern is found, keep `email: null`, note the pattern in evidence or assumptions, and let the `find-person-email` skill report `pattern_observed` after import if useful.
...
8. Validate and dedupe:
   - Run `node "$SKILL_DIR/scripts/validate-prospect-json.js" <prospect.json>` and save the output to the returned `paths.validation`.
   - Run `node "$SKILL_DIR/scripts/check-duplicates.js" <prospect.json> --api-base http://localhost:4200` and save the output to the returned `paths.duplicates`.
9. Import by default:
   ...
   - Run `node "$SKILL_DIR/scripts/upsert-prospects.js" <prospect.json> --api-base http://localhost:4200 --apply` and save the output to the returned `paths.upsert_result`.
...
10. Kick off post-import email lookup:
   ...
   - Read and use the `find-person-email` skill before researching or verifying emails.
...
For artifacts, create machine-readable JSON under the per-run directory in `.agent-runs/prospect-company/`. Keep all intermediate files and logs in that run directory. Do not create or write generated files to `data/`, `/private/tmp`, or a root-level `tmp/` directory. Keep evidence and confidence outside persisted app records unless the schema is extended.
