...
Use `node "$SKILL_DIR/scripts/upsert-prospects.js" <prospect.json> --api-base http://localhost:4200 --apply`. The script uses API reads for duplicate planning, then performs all creates/updates through the API. Start the app with `npm run dev -- --port 4200` when `http://localhost:4200` is unavailable.
